...
## 描述
`Set-NetQosFlowControl` cmdlet 用于设置 Windows Server® 2012 及更高版本中基于优先级的流量控制（PFC）功能的启用状态。如果 Windows Server 2012 或更高版本被设置为“不接受来自远程设备的配置”，那么它会将这些更新后的配置发送到计算机中支持数据中心桥接（DCB）功能的网络适配器上。

有关远程设备配置的更多信息，请参阅 **Set-NetQosDcbxSetting** cmdlet。

基于优先级的流量控制（PFC）是IEEE数据中心桥接（DCB）标准的一部分。
...
此命令可以对标记为优先级 5 的流量启用流控制功能。这个示例等同于使用优先级 5 的 **Enable-NetQosFlowControl** cmdlet。
...
该命令使用 `Get-NetQosFlowControl` cmdlet 来获取带有优先级 5 的流量的流量控制设置。然后，通过管道运算符（pipeline operator）将结果传递给当前的 cmdlet，该 cmdlet 会禁用这些流量控制设置。
...
`Microsoft.ManagementInfrastructure.CimInstance` 对象是一个包装类，用于显示 Windows Management Instrumentation (WMI) 对象。井号（`#`）后面的路径表示底层 WMI 对象的命名空间和类名。此 cmdlet 返回一个 **MSFT_NetQosFlowControlSettingData** 对象，其中包含每个优先级的流量控制设置。只有当你指定了 *PassThru* 参数时，该 cmdlet 才会返回这个 **MSFT_NetQosFlowControlSettingData** 对象。

## 备注

## 相关链接

[禁用 NetQos 流量控制](./ Disable-NetQosFlowControl.md)
