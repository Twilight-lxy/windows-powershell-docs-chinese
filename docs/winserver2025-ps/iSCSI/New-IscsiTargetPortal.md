...
## 描述
`New-IscsiTargetPortal` cmdlet 用于连接一个新的 iSCSI 目标门户（target portal）。

## 示例

### 示例 1：添加一个 iSCSI 目标端口
```
...
此命令添加了一个名为“testIscsi”的新iSCSI目标门户，用于执行发现（即查找可用的iSCSI资源）操作。
...
### -ChapSecret
指定在通过 CHAP 进行身份验证以建立连接时使用的 CHAP 密码。
...
### -IsDataDigest
该参数用于指示当发起者登录目标门户时，此 cmdlet 是否会启用数据摘要功能。如果未指定此参数，则数据摘要功能的设置将由发起者的内核模式驱动程序决定。
...
### -IsHeaderDigest
该参数用于指示当发起者登录目标门户时，此cmdlet是否启用头部摘要（header digest）功能。如果您不指定此参数，则摘要设置将由发起者的内核模式驱动程序决定。
...
### -ThrottleLimit
指定可以同时运行的最大操作数量。如果省略此参数或输入值为 `0`，则 Windows PowerShell® 会根据计算机上正在运行的 CIM cmdlet 数量来计算该 cmdlet 的最佳限制值。此限制仅适用于当前执行的 cmdlet，而不适用于整个会话或计算机本身。
