...
## 描述
`Get-DnsServerTrustPoint` cmdlet 可以获取域名系统（DNS）服务器上一个或多个信任点。如果您没有指定信任点的名称，那么将列出所有的信任点。

## 示例

### 示例 1：获取 DNS 服务器上的信任点列表
```
PS C:\> Get-DnsServerTrustPoint
```

这个命令可以获取本地DNS服务器上所有信任点的列表。

## 参数

### -AsJob
将此cmdlet作为后台作业运行。使用此参数来执行那些需要较长时间才能完成的任务。
...
### Microsoft.ManagementInfrastructure.CimInstance#DnsServerTrustPoint[]
