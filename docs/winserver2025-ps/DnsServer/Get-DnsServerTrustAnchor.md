...
## 描述
`Get-DnsServerTrustAnchor` cmdlet 可用于获取域名系统（DNS）服务器上的信任锚点。如果您没有指定要获取的信任锚点的名称，该 cmdlet 会返回所有信任锚点。

## 示例

### 示例 1：获取某个信任区域内的所有信任锚点
```
PS C:\> Get-DnsServerTrustAnchor -Name "sec.contoso.com"
```

此命令用于获取名为 sec.contoso.com 的信任区域的信任锚点（trust anchors）。
...
该cmdlet会立即返回一个表示该作业的对象，随后会显示命令提示符。在作业完成之前，您可以继续在该会话中操作。要管理作业，请使用`*-Job`系列的cmdlets；要获取作业结果，请使用[Receive-Job](https://go.microsoft.com/fwlink/?LinkID=113372) cmdlet。
...
### -ComputerName
指定一个远程DNS服务器。此参数的可接受值为：IP地址，或任何能够解析为IP地址的值，例如完全限定域名（FQDN）、主机名或NETBIOS名称。
...
### -ThrottleLimit
该参数用于指定可以同时进行的操作的最大数量，以便运行相应的 Cmdlet。如果省略此参数或输入值为 `0`，那么 Windows PowerShell® 会根据计算机上正在运行的 CIM Cmdlet 的数量来计算出一个最优的并发限制。该并发限制仅适用于当前正在运行的 Cmdlet，而不适用于整个会话或计算机本身。
