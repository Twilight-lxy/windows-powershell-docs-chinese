...
## 描述
`Show-DNSServerCache` cmdlet 会以以下格式显示所有缓存的域名系统（DNS）服务器资源记录：名称、资源记录数据（ResourceRecordData）以及生存时间（Time-to-Live，简称 TTL）。

## 示例

#### 示例 1：显示 DNS 服务器缓存中的所有资源记录
```
