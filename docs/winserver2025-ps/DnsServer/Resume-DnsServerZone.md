...
## 描述
`Resume-DnsServerZone` cmdlet 用于恢复已暂停的域名系统（DNS）区域的名称解析功能。您可以使用 `Suspend-DnsServerZone` cmdlet 来暂时阻止 DNS 服务器响应对该区域的查询请求。
...
此命令用于恢复名为“western.contoso.com”的暂停区域。该命令使用了*PassThru*参数来生成输出结果。
...
### -Force
它会自动恢复名称解析功能，而无需提示您进行确认。默认情况下，该 cmdlet 在执行之前会先请求您的确认。
...
### -ThrottleLimit
指定可以同时建立的最大并发操作数量，以便运行该cmdlet。如果省略此参数或输入`0`，则Windows PowerShell®会根据计算机上正在运行的CIM cmdlet的数量来计算该cmdlet的最优限制值。此限制仅适用于当前执行的cmdlet，而不适用于整个会话或计算机本身。
