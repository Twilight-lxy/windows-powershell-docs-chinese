...
`Get-DedupStatus` cmdlet 为每个具有数据去重元数据的卷返回一个去重状态对象。一个 `DeduplicationStatus` 对象包含只读属性，这些属性描述了该卷的容量、可用空间或已用空间、优化节省情况以及当前状态；同时还包括该卷上最近执行的作业的相关信息（如作业开始和结束的时间）及完成状态。
...
在远程会话或远程计算机上运行该 cmdlet。可以输入一个计算机名称，或者提供一个会话对象（例如通过 [New-CimSession](/powershell/module/cimcmdlets/new-cimsession) 或 [Get-CimSession](https://go.microsoft.com/fwlink/p/?LinkId=227966) cmdlet 所获得的会话对象）。默认情况下，该 cmdlet 会在本地计算机的当前会话中执行。
...
指定一个或多个文件系统卷，以返回相应的 DeduplicationStatus 对象。输入一个或多个卷 ID、驱动器字母或卷 GUID 路径。对于驱动器字母，请使用格式 `D:`；对于卷 GUID 路径，请使用格式 `\\?\Volume{{GUID}}`。多个卷之间用逗号分隔。
...
### Microsoft.ManagementInfrastructure.CimInstance#ROOT/Microsoft/Windows/Deduplication/MSFT_DedupVolumeStatus
