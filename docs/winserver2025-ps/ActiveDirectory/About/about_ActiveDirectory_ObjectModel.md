---  
标题：关于 ActiveDirectory 对象模型  
日期：2013年4月22日  
描述：介绍了用于 Windows PowerShell 的 Active Directory 模块的对象模型。  
语言环境：en-US  
模式版本：2.0.0  

---

# 关于 ActiveDirectory 对象模型

## 简要描述
描述了用于 Windows PowerShell 的 Active Directory 模块的对象模型。

## 长描述

这个主题解释了Active Directory模块中的类及其属性，这些类和属性用于模拟实际的Active Directory属性。同时，它还概述了由Active Directory对象对应的类构成的层次结构。该对象模型为Active Directory模块cmdlet所支持的所有操作提供了数据基础。

### 类层次结构

以下列表展示了在 Active Directory 模块对象模型中定义的类层次结构，类的继承关系通过缩进来表示。这种继承模型使得 Active Directory cmdlet 能够接受多种类型的对象作为输入。例如，Get-ADPrincipalGroupMembership cmdlet 可以接受以下任何一种对象作为输入：ADGroup、ADAccount、ADComputer、ADServiceAccount 或 ADUser。之所以能够实现这一点，是因为类的继承关系；这种关系确保了 ADUser 对象具备所有 ADPrincipal 对象的属性。
...
### Active Directory 模块类

以下列表展示了类层次结构中所有的 Active Directory 模块类。每个类都定义了一组属性，其中一些是默认获取的 LDAP 属性，而另一些则是专门为 Active Directory 模块创建的新属性。这些新属性是从一个或多个 LDAP 属性派生而来的，具体细节可以在相应的类描述中找到。
