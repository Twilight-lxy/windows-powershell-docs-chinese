...
# 设置 AD 详细密码策略（Set-ADFineGrainedPasswordPolicy）
...
## 描述
`Set-ADFineGrainedPasswordPolicy` cmdlet 用于修改 Active Directory 中的细粒度密码策略的属性。您可以通过使用 cmdlet 参数来修改常用的属性值；对于那些未与 cmdlet 参数关联的属性值，可以使用 `Add`、`Replace`、`Clear` 和 `Remove` 参数来进行修改。

`Identity` 参数用于指定要修改的 Active Directory 细粒度密码策略。您可以通过该策略的 distinguished name、GUID 或名称来识别它。您也可以将 `Identity` 参数设置为某个对象变量（例如 `$<localFineGrainedPasswordPolicyObject>`），或者通过管道将一个对象传递给 `Identity` 参数。例如，您可以使用 **Get-ADFineGrainedPasswordPolicy** cmdlet 获取一个细粒度密码策略对象，然后通过管道操作符将该对象传递给 **Set-ADFineGrainedPasswordPolicy** cmdlet。

`Instance` 参数提供了一种通过应用对对象副本所做的更改来更新细粒度密码策略对象的方法。当您将 `Instance` 参数设置为已修改的 Active Directory 细粒度密码策略对象的副本时，`Set-ADFineGrainedPasswordPolicy` cmdlet 会对原始的细粒度密码策略对象执行相同的更改。要获取用于修改的对象副本，请使用 `Get-ADFineGrainedPasswordPolicy` 命令。在使用 `Instance` 参数时，不允许同时使用 `Identity` 参数。有关 `Instance` 参数的更多信息，请参阅其相关描述。

## 示例

### 示例 1：更新细粒度密码策略对象上的属性
```
...
此命令会更新名为“MyPolicy”的`FineGrainedPasswordPolicy`对象中的以下属性：`Precedence`、`LockoutDuration`、`LockoutObservationWindow`、`ComplexityEnabled`、`ReversibleEncryptionEnabled`以及`MinPasswordLength`。

#### 示例 2：使用区分名称（Distinguished Name）在细粒度密码策略中设置某个属性
```
...
此命令用于设置名为 CN=MyPolicy,CN=Password Settings Container,CN=System,DC=USER01,DC=COM 的 **FineGrainedPasswordPolicy** 对象上的 **MinPasswordLength** 属性。

### 示例 3：获取详细的密码策略，然后更新一组属性
```
...
这个示例获取名为“MyPolicy”的**FineGrainedPasswordPolicy**对象，更新该对象上的一组属性，然后使用*Instance*参数将这些修改内容写回目录中。
...
此命令用于修改名为 Level3Policy 的细粒度密码策略的 **Precedence**（优先级）属性。该命令首先使用 **Get-ADFineGrainedPasswordPolicy** cmdlet 获取相应的细粒度密码策略，然后通过管道运算符将该策略传递给当前的 cmdlet。

## 参数

### -Add
用于指定要添加到对象属性中的值。使用此参数可以为无法通过 cmdlet 参数修改的属性添加一个或多个值。若要修改对象属性，必须使用轻量级目录访问协议（LDAP）显示名称。可以通过提供用逗号分隔的值列表来为单个属性指定多个值；若需要为多个属性分别设置值，则可以使用分号进行分隔。该参数的格式如下：

`-添加以下内容：@{Attribute1LDAPDisplayName=value1, value2, ...; Attribute2LDAPDisplayName=value1, value2, ...; AttributeNLDAPDisplayName=value1, value2, ...}`
