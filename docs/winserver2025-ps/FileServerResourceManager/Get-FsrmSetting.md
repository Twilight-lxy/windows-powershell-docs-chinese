...
## 摘要
获取当前的FSRM（File System Reporting Manager）设置。
...
## 描述
`Get-FsrmSetting` cmdlet 用于检索当前的 FSRM（文件系统资源管理）设置。您可以使用 `Set-FsrmSetting` cmdlet 来修改 FSRM 设置。
...
这个命令用于获取当前的FSRM（File Server Resource Manager）设置。
...
### -ThrottleLimit
指定可以同时运行的最大并发操作数量。如果省略此参数或输入值`0`，则 Windows PowerShell® 会根据计算机上正在运行的 CIM cmdlet 的数量来计算该 cmdlet 的最佳限制值。此限制仅适用于当前运行的 cmdlet，而不适用于整个会话或计算机本身。
