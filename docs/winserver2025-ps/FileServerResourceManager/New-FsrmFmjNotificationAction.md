...
## 描述
`New-FsrmFmjNotificationAction` cmdlet 返回一个通知动作对象，您可以使用该对象来创建文件管理作业动作。您可以将 `FsrmFMJNotificationAction` 对象作为输入参数，用于以下 File Server Resource Manager (FSRM) cmdlets：
