...
## 描述
`Add-VMScsiController` cmdlet 可以在虚拟机中添加一个 SCSI 控制器。
...
### -ComputerName
指定一个或多个用于添加 SCSI 控制器的 Hyper-V 主机。可以使用 NetBIOS 名称、IP 地址或完全限定域名进行标识。默认值是本地计算机。可以使用 `localhost` 或点（.）来明确表示本地计算机。
...
### -Passthru
指定需要将一个 **VMScsiController** 对象传递给管道中，该对象代表要添加的 SCSI 控制器。
