...
## 摘要
获取Hyper-V主机上可用的虚拟存储区域网络（SAN）信息。
...
## 描述
`Get-VMSan` cmdlet 可以获取一个或多个 Hyper-V 主机上可用的虚拟存储区域网络（SAN）。
...
获得了一个名为“ProductionSAN”的虚拟存储区域网络（SAN）。
...
### -ComputerName
指定一个或多个 Hyper-V 主机，从中获取可用的虚拟机存储区域网络（SAN）。允许使用 NetBIOS 名称、IP 地址和完全限定域名。默认值为本地计算机。可以使用 `localhost` 或点号（.）来明确表示本地计算机。
