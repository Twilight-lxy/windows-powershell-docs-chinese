...
## 描述
`Get-VMSwitchExtensionPortData` cmdlet 可以检索应用于虚拟网络适配器的虚拟交换机扩展功能的状态。这些端口功能数据会提供每个端口的运行时信息和统计信息。
...
从虚拟机VM1中的虚拟网络适配器上的示例转发扩展程序中，获取ID为eb29f0f2-f5dc-45c6-81bb-3cd9f219bbbb的端口特性数据。
...
### -ComputerName
用于指定一个或多个 Hyper-V 主机，以获取应用于虚拟网络适配器的虚拟交换扩展的状态信息。允许使用 NetBIOS 名称、IP 地址或完全限定域名作为主机标识。默认值为本地计算机；可以使用 `localhost` 或点（.）来明确表示本地计算机。
...
### -VM
指定一个虚拟机对象数组。该cmdlet会获取你所指定的虚拟机的虚拟交换机扩展的状态。要获取一个虚拟机对象，请使用**Get-VM** cmdlet。
