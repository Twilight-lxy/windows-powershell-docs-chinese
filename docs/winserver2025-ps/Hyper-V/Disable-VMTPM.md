...
## 描述
`Disable-VMTPM` cmdlet 用于禁用虚拟机上的可信平台模块（Trusted Platform Module, TPM）功能。

## 示例

#### 示例 1：在虚拟机上禁用 TPM（Trusted Platform Module）
```
PS C:\> Disable-VMTPM -VM $myVM
```

此命令会禁用存储在 `$myVM` 变量中的虚拟机上的 TPM（Trusted Platform Module，可信平台模块）。
...
### -ComputerName
指定一个或多个用于运行该 cmdlet 的 Hyper-V 主机。可以使用 NetBIOS 名称、IP 地址或完全限定域名进行指定。默认值为本地计算机。可以使用 “localhost” 或点 (“.”) 明确指定本地计算机。
