...
## 描述
`Get-VMIntegrationService` cmdlet 可以获取虚拟机或快照的集成服务信息。
...
从虚拟机TestVM获取关机（Shutdown）和VSS集成服务。
...
在应用虚拟机 TestVM 的更新之前，从快照中获取 Shutdown 和 VSS 集成服务。
...
### -ComputerName
指定一个或多个用于获取集成服务的 Hyper-V 主机。允许使用 NetBIOS 名称、IP 地址和完全限定域名。默认值为本地计算机。可以使用 `localhost` 或点号（.）来明确表示本地计算机。
...
### Microsoft.HyperV POWERShel.VMIntegrationComponent
