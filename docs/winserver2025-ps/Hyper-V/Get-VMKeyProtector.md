...
## 描述
`Get-VMKeyProtector` cmdlet 用于获取虚拟机的密钥保护器（key protector）。
...
### -ComputerName
指定一个或多个用于运行该 cmdlet 的 Hyper-V 主机。可以使用 NetBIOS 名称、IP 地址或完全限定域名进行识别。默认值为本地计算机。可以使用 “localhost” 或点 (“.”) 显式指定本地计算机。
...
### -VMName
指定该虚拟机的名称；此虚拟机将用于获取密钥保护器（key protector）。
