...
## 描述
`Get-VMSystemSwitchExtension` cmdlet 可以获取安装在虚拟机主机上的交换扩展程序（switch extensions）。返回的交换扩展程序不包含用于实现某些功能的嵌入式对象，也不包含功能 ID 的数组。
...
### -ComputerName
指定一个或多个用于获取交换机扩展程序的 Hyper-V 主机。允许使用 NetBIOS 名称、IP 地址和完全限定域名作为主机标识。默认值为本地计算机。可以使用 “localhost” 或点号（.）来明确表示本地计算机。
