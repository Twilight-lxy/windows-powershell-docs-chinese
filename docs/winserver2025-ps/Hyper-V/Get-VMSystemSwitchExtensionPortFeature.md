...
## 描述
`Get-VMSystemSwitchExtensionPortFeature` cmdlet 可用于获取一个或多个 Hyper-V 主机上虚拟交换机扩展所支持的端口级功能。返回的函数对象将包含这些功能的默认值。该对象可以通过 `Add-VmSwitchExtensionPortFeature` 命令应用于特定的端口上。
...
可以获取所有端口级别的功能，这些功能由系统上安装的各种虚拟交换机扩展程序提供支持，并且可以在 Hyper-V 的虚拟网络适配器上进行配置。
...
### -ComputerName
指定一个或多个 Hyper-V 主机，以从中获取可用的端口级功能。允许使用 NetBIOS 名称、IP 地址和完全限定域名。默认值是本地计算机。可以使用 `localhost` 或点（.）来明确表示本地计算机。
