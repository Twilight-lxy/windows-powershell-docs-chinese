...
# 设置 Set-BCDataCacheEntryMaxAge 的值
...
## 描述
`Set-BCDataCacheEntryMaxAge` cmdlet 指定了数据可以在缓存中保留的最长时间。

## 示例

### 示例 1：指定内容的最大年龄（即内容创建的时间上限）
```
PS C:\> Set-BCDataCacheEntryMaxAge -TimeDays 100
```

这个例子规定：内容信息及其相关数据将在缓存中保留100天。100天后，这些内容将被从缓存中删除。
...
### -ThrottleLimit
该参数用于指定可以同时运行的最大并发操作数量。如果省略此参数或输入值 `0`，则 Windows PowerShell® 会根据计算机上正在运行的 CIM cmdlet 的数量来计算 해당 cmdlet 的最佳限制值（即并发操作的最大数量）。此限制仅适用于当前的 cmdlet，而不适用于整个会话或计算机本身。
...
### -TimeDays
指定内容信息和相关数据在缓存中保留的天数，超过这个时间后才会被删除。
