...
# 禁用 NetAdapterEncapsulatedPacketTask 的卸载功能
...
## 描述
`Disable-NetAdapterEncapsulatedPacketTaskOffload` 这个 cmdlet 用于禁用封装数据包任务的卸载功能。该卸载功能允许网络适配器对封装数据包的头部执行诸如大型数据包发送（LSO: Large Send Offload）和虚拟机队列（VMQ: Virtual Machine Queue）等操作。运行此 cmdlet 可能会导致网络性能下降。

## 示例

### 示例 1：禁用指定网络适配器的封装数据包任务卸载功能
```
PS C:\> Disable-NetAdapterEncapsulatedPacketTaskOffload -Name "MyAdapter "
```

此命令将禁用名为“MyAdapter”的网络适配器的封装数据包任务卸载功能，并重新启动该网络适配器。
...
### -IncludeHidden
表示此 cmdlet 包含可见和隐藏的网络适配器。默认情况下，仅包含可见的网络适配器。如果在识别网络适配器时使用了通配符，并且指定了此参数，则该通配符字符串将同时与隐藏和可见的网络适配器进行匹配。
...
指定一个网络适配器接口描述数组。对于物理网络适配器而言，这通常是网络适配器制造商的名称后加上型号和描述，例如 `Contoso 12345 千兆网络设备`。
...
### -ThrottleLimit
该参数用于指定可以同时运行的cmdlet的最大操作数量。如果省略此参数或输入值为`0`，则Windows PowerShell®会根据计算机上正在运行的CIM cmdlet的数量来计算该cmdlet的最佳限制值。此限制仅适用于当前的cmdlet，而不适用于整个会话或计算机本身。
...
`Microsoft.ManagementInfrastructure.CimInstance` 对象是一个包装类，用于显示 Windows 管理规范（WMI）对象。井号（`#`）之后的路径指定了底层 WMI 对象的命名空间和类名。
