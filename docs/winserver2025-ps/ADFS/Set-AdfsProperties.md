...
## 描述
`Set-AdfsProperties` cmdlet 用于设置控制 Active Directory Federation Services (AD FS) 中全局行为的属性。
...
第一个命令创建了一个 **TimsSpan** 对象，并将结果存储在名为 `$Timespan` 的变量中。

第二个命令设置了一个外部网络（extranet）的锁定机制，该机制规定：在用户连续输入4次错误的密码后，系统将立即对其账户进行锁定。此外，这个命令还设置了用于存储在$Timespan变量中的值的观察窗口（observation window）。

## 参数

### -AcceptableIdentifiers
指定一个标识符数组，这些标识符可以作为联邦服务（Federation Service）在验证从其他声明提供者（claims provider）收到的声明时所使用的名称。
...
指定用于连接数据库的连接字符串，该数据库保存着工件解析服务所使用的各种工件信息。
