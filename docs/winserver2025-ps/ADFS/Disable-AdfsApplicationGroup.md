...
## 描述
` Disable-AdfsApplicationGroup ` 这个 cmdlet 用于禁用 Active Directory Federation Services (AD FS) 中的一个应用程序组。
...
当指定 `PassThru` 参数时，会返回一个已被禁用的 `ApplicationGroup` 对象。默认情况下，此 cmdlet 不会产生任何输出。
