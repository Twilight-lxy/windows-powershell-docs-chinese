...
## 描述
`Remove-AdfsApplicationGroup` cmdlet 用于从 Active Directory Federation Services (AD FS) 中删除一个应用程序组。
...
### Microsoft IdentityServer.ManagementResources.ApplicationGroup

`ApplicationGroup` 对象通过 `TargetApplicationGroup` 参数被接收。

### System.String

字符串对象通过 *TargetApplicationGroupIdentifier* 和 *TargetName* 参数被接收。

## 输出

### Microsoft IdentityServer.ManagementResources.ApplicationGroup

当指定 `PassThru` 参数时，会返回被删除的 `ApplicationGroup` 对象。默认情况下，此cmdlet不会生成任何输出。
