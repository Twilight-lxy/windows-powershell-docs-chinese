...
## 描述
`Remove-DhcpServerv4Scope` cmdlet 用于从动态主机配置协议（DHCP）服务器服务中删除指定的 IPv4 范围。
...
这个示例会从DHCP服务器服务中删除指定的作用域（scopes）。

### 示例 2：在不进行确认的情况下删除作用域
```
PS C:\> Remove-DhcpServerv4Scope -ComputerName "dhcpserver.contoso.com" -ScopeId 10.10.10.0, 10.20.20.0 -Force
```

这个示例会从DHCP服务器服务中删除指定的作用域（scopes）。即使这些作用域处于活动状态或其中包含正在使用的客户端租约（client leases），该cmdlet也不会提示用户进行确认。
...
这个示例会删除DHCP服务器服务上所有被禁用的作用域。`Get-DhcpServerv4Scope` cmdlet用于获取作用域对象，并将这些对象传递给`Where-Object` cmdlet。`Where-Object` cmdlet会筛选出状态为“InActive”（即已被禁用）的作用域对象，然后把这些被筛选出的对象传递给另一个cmdlet，该cmdlet负责删除这些被禁用的作用域。
...
### -Force
表示此 cmdlet 会删除某个作用域，即使该作用域中包含正在使用的租约（active leases）。
...
### -ScopeId
指定一个或多个以 IPv4 地址格式表示的作用域标识符（ID），以便将其删除。
