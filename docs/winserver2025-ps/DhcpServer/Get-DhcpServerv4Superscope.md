...
## 描述
`Get-DhcpServerv4SuperScope` cmdlet 用于获取指定超范围的配置信息。如果您没有指定 `SuperscopeName` 参数，那么系统会返回动态主机配置协议（DHCP）服务器服务上存在的所有超范围的信息。

## 示例

### 示例 1：获取所有的超作用域（superscopes）及其关联的作用域（scopes）
```
PS C:\> Get-DhcpServerv4SuperScope -ComputerName "dhcpserver.contoso.com"
```

这个示例会获取指定DHCP服务器服务上的所有超作用域（superscopes）及其相关的普通作用域（associated scopes）。

#### 示例 2：获取一个超范围（superscope）及其关联的范围
```
PS C:\> Get-DhcpServerv4SuperScope -ComputerName "dhcpserver.contoso.com" -SuperscopeName "ExtendedLabNet"
```

这个示例从名为dhcpserver.contoso.com的计算机上，获取名为ExtendedLabNet的超范围的配置信息以及该超范围所关联的所有子范围的配置信息。这些配置数据来自指定的DHCP服务器服务。
...
### -SuperscopeName
指定该cmdlet获取其配置的超作用域（superscope）的名称。
