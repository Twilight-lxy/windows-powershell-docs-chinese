...
## 描述
`Get-DhcpServerv4ExclusionRange` cmdlet 用于返回从指定的作用域标识符（IDs）中排除的 IPv4 地址范围。如果未指定 `ScopeId` 参数，则会返回动态主机配置协议（DHCP）服务器服务上所有被排除的 IPv4 地址范围。
...
这个示例会获取在名为 dhcpserver.contoso.com 的计算机上运行的指定 DHCP 服务器服务中所有被排除的 IPv4 地址范围。

### 示例 2：从 DHCP 服务器服务的作用域中获取所有被排除的地址范围
```
PS C:\> Get-DhcpServerv4ExclusionRange -ComputerName "dhcpserver.contoso.com" -ScopeId 10.10.10.0
```

这个示例会获取在名为 dhcpserver.contoso.com 的计算机上运行的指定 DHCP 服务器服务中、位于指定范围内的所有被排除的 IPv4 地址段。

## 参数

### -AsJob
将该cmdlet作为后台作业运行。使用此参数来执行需要较长时间才能完成的命令。该cmdlet会立即返回一个表示该作业的对象，然后显示命令提示符。在作业完成期间，您可以继续在当前会话中工作。要管理该作业，请使用`*-Job`系列的cmdlets。要获取作业结果，请使用[Receive-Job](https://go.microsoft.com/fwlink/?LinkID=113372) cmdlet。有关Windows PowerShell®后台作业的更多信息，请参阅[about_Jobs](https://go.microsoft.com/fwlink/?LinkID=113251)。
...
### -ScopeId
指定用于返回被排除的 IP 地址范围的地址范围 ID（采用 IPv4 地址格式）。
